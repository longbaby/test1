...
public class EBankMobileLogin extends HttpServlet {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private final static Logger logger = LoggerFactory
			.getLogger(EBankMobileLogin.class);
	@Override
...
//		String casClientMobileUrl = "";
		String casClientMobileUrl = request.getParameter("service");
		
		/*if("pad".equals(mobileType)){
...
//		
//		map1.put("sceneType", SceneType.LOGIN_AFTER_EBANKP_PHONE);
		String sceneType = request.getParameter("sceneType");
		String mobileType = request.getParameter("mobileType");
		logger.error("request.getParameter-----sceneType="+sceneType+"; mobileType="+mobileType);	
		if ((sceneType!=null) && (sceneType!="")) {
			map1.put("sceneType", sceneType);
		} else {
			if("pad".equals(mobileType)){
				map1.put("sceneType", SceneType.LOGIN_AFTER_EBANKP_PAD);
			}else{
				map1.put("sceneType", SceneType.LOGIN_AFTER_EBANKP_PHONE);
			}
...
//		map1.put("loginWay", "1");
		map1.put("loginChannels", LoginChannels.MOVEINTERNET);
		map1.put("service", casClientMobileUrl);
		
		JSONObject obj = JSONObject.fromObject(map1);
		eMallFieldValue = obj.toString();
		System.out.println("eMallField="+eMallFieldValue);
		logger.error("eMallFieldValue="+eMallFieldValue);
		...
		String logonCardNum = request.getParameter("logonCardNum");
		String logonCardPass = request.getParameter("logonCardPass");
		...
		logger.error("cas.mobile.eBanklogin:"+casEBankLoginUrl);
...
//		.append("1").append("&netType=").append("111");
		...
		.append("&changerule=").append(java.net.URLEncoder.encode(changerule, "UTF-8"))
		...
		logger.error("cas.mobile.eBanklogin&redirectUrl:"+redirectUrl.toString());
